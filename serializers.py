...
class LPRSerializers(serializers.Serializer):
    input_folder = serializers.PrimaryKeyRelatedField(queryset=Folder.objects.all(), allow_null=True)

    def create(self, validated_data):
        input_folder = validated_data["input_folder"]
        output_dir = "/home/itwatcher/tricycle_copy/lpr_output"
        with tempfile.TemporaryDirectory() as tmpdir:
            command = ["python", "lpr.py", f"--input_folder={validated_data['input_folder']}", f"--output_folder={tmpdir}"]
            result = run(command, capture_output=True, text=True)
            if result.returncode != 0:
                raise Exception(f"Error running lpr.py: {result.stderr.strip()}")
            # Copy the output images to the output directory
            for image_file in os.listdir(tmpdir):
                shutil.copyfile(os.path.join(tmpdir, image_file), os.path.join(output_dir, image_file))
        return output_dir'''
