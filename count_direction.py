...
from core.config_all import cfg
...
class VehiclesCounting():
    def __init__(self, file_counter_log_name, framework='tf', weights='./checkpoints/catch_all',
            ...
            if return_value:
                ...
                if total_frames % skip_frames == 0:
                   ...
                   #oval
                    ...
                    # Create a list of lines and their corresponding directions.
                    lines = [line1, line2, line3, line4]
                    #directions = ["to_hwy", "to_msu", "to_oval", "to_sm"]
                    # Initialize passed_counter as a nested dictionary
                    #passed_counter = {direction: {} for direction in directions} #w/ class_name
                    ...
                    for track in tracker.tracks:
                        ...
                        # Check for an intersection between midpoint and the current line.
                        #if self._intersect(midpoint, previous_midpoint, line1[0], line1[1]) and track.track_id not in already_counted:
                        #if distance <= desired_distance_range and track.track_id not in already_counted:
                        ...
                        for line, direction in zip(lines, passed_counter.keys()):
                            if self._intersect(midpoint, previous_midpoint, line[0], line[1]) and track.track_id not in already_passed:
                                #passed_counter[direction][class_name] = passed_counter[direction].get(class_name, 0) + 1
                                #passed_counter[direction][class_name] = 0 #w/ classname
                                ...
                                if direction == "Going National Highway" and angle > 0:
                                    to_hwy += 1
                                elif direction == "Going MSU CETD" and angle < 0:
                                     to_msu += 1
                                elif direction == "Going SM" and angle < 0:
                                    to_oval += 1
                                elif direction == "Going Oval Plaza" and angle > 0:
                                    to_sm += 1
                        ...
                        cv2.putText(frame, str(class_name), (int(bbox[0]), int(bbox[1])), 0,
                                        1.1e-3 * frame.shape[0], (0, 0, 255), 2)
                    ...
                    for cls in class_counter:
                        ...
                        cv2.putText(frame, str(cls) + " " + str(class_count), (int(0.02 * frame.shape[1]), int(y)), 0,
